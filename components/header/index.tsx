...
function Index({
  ...
  useEffect(() => {
    ...
    window.addEventListener("click", (e) => {
      if (!buttonMenu?.current?.contains(e.target as Node)) setOpen(false);
    });
  }, []);
  const updatePositionEachElement = (currentUrl: string) => {
    switch (currentUrl) {
      case "/album":
        setAction((): any => {
          const offsetTop: any = album?.current?.offsetTop;
          return { y: offsetTop || 0, action: 1 };
        });
        break;
      case "/result":
        setAction((): any => {
          const offsetTop: any = result?.current?.offsetTop;
          return { y: offsetTop || 50, action: 2 };
        });
        break;
      case "/face":
        setAction((): any => {
          const offsetTop: any = face?.current?.offsetTop;
          return { y: offsetTop || 100, action: 3 };
        });
        break;
      case "/map":
        setAction((): any => {
          const offsetTop: any = map?.current?.offsetTop;
          return { y: offsetTop || 150, action: 4 };
        });
        break;
      case "/setting":
        setAction((): any => {
          const offsetTop: any = setting?.current?.offsetTop;
  ...
  return (
    <nav>
      {screen >= 768 ? (
        ...
        <div
          ref={buttonMenu}
          className="md:hidden sm:top-[50px] sm:left-[20px]  flex absolute"
        >
          <motion.div
          ...
          >
            ...
            <ul
              className={`${
                isOpen ? "flex" : "hidden"
              }  flex-col min-h-[100px] relative w-full items-center justify-between bg-[#202020] h-full xl:w-[160px] lg:w-[140px] md:w-[120px] max-h-[750px]`}
            >
              <motion.div
                animate={"animetion"}
                custom={action}
